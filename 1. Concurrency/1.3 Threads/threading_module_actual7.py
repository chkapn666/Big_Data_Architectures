...
# Now there is a common resource that requires not exclusive access of 
# a single thread; rather, multiple ones (but a limited number) can access it once
semaphore = threading.BoundedSemaphore(value=3)

def access(thread_number):  # it is good to retain a unique identifier per thread to know which
    # one is trying to access our resource
